...
<div v-if="viewNewProduct">
    <div class="p-4 text-lg text-white bg-gray-800">
        <strong class="float-left">CREAR PRODUCTO</strong>
...
export default {
    setup()
    {
        ...
        EventBus.on('openUpdateProduct', (category) =>{
            viewNewProduct.value = false;
            reset(true);
        });

        EventBus.on('CategoryActive', (category) =>{
            const cadena = '{"'+category+'":true}';
        ...
        const save = async()=>{
            ...
            let category = Object.keys(category_id.value)[0];
            ...
            {
                /////TableProducts
                EventBus.emit('NewProduct',response.data);
                reset();
            })
            .catch(error=>{
