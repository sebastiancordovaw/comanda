<template>
    ...
    <div class="grid grid-cols-12 p-4 pb-4 mt-2 gap-y-2">
        ...
        <div class="sm:col-span-12" v-if="!sonCategories">
            <input
            ...
            <div class="mt-2">
                <select v-model="fatherCategory" :disabled="(isSubCategory)?false:true" id="name" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-orange-400 sm:text-sm sm:leading-6">
                    ...
                    <template v-for="(cat, i ) in selectCategories" :key="i">
                        <option v-if="category.id != cat.id"  :value="cat.id">{{ cat.name }}</option>
                    </template>
                </select>
            </div>
        </div>
...
export default {
    ...
    setup(props,{emit}){
        ...
        onMounted(()=>{
            ...
            setTimeout(() => {
                qr.value = props.category.qr;
                if(props.category.category_id)
                {
                    isSubCategory.value = 1;
                    fatherCategory.value = props.category.category_id;
                }

            }, 1);
            errores.value = "";

            if(typeof props.category.categories == 'object')
            {
                sonCategories.value = props.category.categories.length
            }
            else
            {
                sonCategories.value=0;
            }
