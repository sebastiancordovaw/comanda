    ...
    <modal  :show = "show" @close="ChangeStateModal">
        ...
        <div v-show="$store.state.products.length">
            <template v-for="(product) in $store.state.products" :key  = "product.id">
                <div v-if="product.date_pay!=null" class="grid grid-cols-12 bg-gray-100 hover:bg-yellow-300 border-orange-700 mt-1 border-l-4 product"  :id="product.id" :class ="product.status?'':'delete'" :paid = "(product.date_pay!=null)?true:false" :is_percentage = "(product.is_percentage)?true:false" :date_pay="product.date_pay">
