<template>
    ...
    <div v-if ="$store.state.cart1[$store.state.tableActivate]" >
        ...
        <div class="grid grid-cols-12 p-1 mt-1 bg-gray-200">
            <div v-if="device.isDesktop"><button @click = "decrease(product)" class="w-full px-3 py-1 font-bold text-white bg-gray-800 rounded-l-sm">-</button></div>
            ...
            <div v-if="device.isDesktop"><button @click = "inclement(product)" class="w-full px-3 py-1 font-bold text-white bg-gray-800 rounded-r-sm">+</button></div>
            ...
            <div v-if="device.isDesktop" class="items-center pt-2 ml-3"><svg @click="commentProduct(product)" xmlns="http://www.w3.org/2000/svg" style="margin:0 auto; position: relative; cursor: pointer;" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5  text-gray-600 hover:text-gray-900">

            <path stroke-linecap="round" stroke-linejoin="round" d="M8.625 9.75a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375m-13.5 3.01c0 1.6 1.123 2.994 2.707 3.227 1.087.16 2.185.283 3.293.369V21l4.184-4.183a1.14 1.14 0 01.778-.332 48.294 48.294 0 005.83-.498c1.585-.233 2.708-1.626 2.708-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0012 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z" />
            </svg>
            </div>
            <div v-if="device.isDesktop" class="items-center pt-2 ml-3 "><svg @click="del(product)" xmlns="http://www.w3.org/2000/svg" style="margin:0 auto; position: relative; cursor: pointer;" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-center text-gray-600 hover:text-gray-900">
    ...
    <div v-show="total>0" class="p-2 mt-1 text-white bg-gray-800">Total a confirmar ${{ total }}</div>
    <div v-show="total>0" class="my-2 ml-5">
        <button @click = "cancelCart1" class="p-2 mr-4 text-white bg-cyan-400 rounded-sm hover:bg-cyan-500 ">Cancelar</button>
        <button @click = "confirmCart1" :disabled="$store.state.loadData" class="p-2 mr-4 text-white bg-gray-800 rounded-sm hover:bg-gray-900 ">Confirmar</button>
    </div>
