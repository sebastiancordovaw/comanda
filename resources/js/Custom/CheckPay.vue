            ...
            <div>
                    ...
                    <template  v-for="(product) in $store.state.products" :key ="product.id">
                        <div v-if ="product.status>0" class="grid grid-cols-12 border-l-2 border-orange-600" :class="(product.date_pay!=null)?'pay':''" :id="'row_check_pay_'+product.id" style="margin: 2px 0 0 0;">
                            ...
                            <div class="checkbox col-span-1 text-right py-2 px-6"><input @click="totalCheck" type="checkbox" :class="(product.date_pay!=null)?'paid':'nopaid'" :id="product.id" :checked="(product.date_pay!=null)?true:false" :disabled="(product.date_pay!=null)?true:false" :value="((product.amount) - (product.amount)*(product.percentage/100)) - $store.state.discount_active/$store.state.products.length" ></div>
                        </div>
                    </template> 
...
export default {
    ...
    setup(props,{emit}){
        ...
        return {idSelect,closeOrderCheck,closemodalcloseOrder,modalClose,modalCloseFinish,totalCheck,tip}
    },
    data()
    {
        return{
