<template>
    <div v-show="$store.state.products.length" style="position: relative;">
        ...
        <div v-if="product.date_pay==null" class="grid grid-cols-12 mt-1 bg-gray-100 border-l-4 border-yellow-400 hover:bg-yellow-200 product"  :id="product.id" :class ="product.status?'':'delete'" :paid = "(product.date_pay!=null)?true:false" :is_percentage = "(product.is_percentage)?true:false" :date_pay="product.date_pay">
            ...
            <div class="col-span-3 p-2 text-xs text-right lg:col-span-2 md:text-md lg:text-base"><span :class="(product.percentage)?'line-through text-gray-600':''"> $ {{ product.amount }}</span> <br/> <span class="font-bold text-red-600" v-if="product.percentage">$ {{ product.amount - (product.amount * product.percentage / 100)}}</span></div>
        ...
        <!--
        <div  class="grid grid-cols-12 mt-1 bg-gray-100 border-l-4 border-green-700 hover:bg-green-300">
            <div class="col-span-3 p-2 "><b class="text-green-600">Subtotal</b></div>
            <div class="col-span-5 p-2"></div>
            <div class="col-span-3 p-2 text-center"><b class="text-green-600">$ {{ $store.state.subtotal_amount_order }}</b></div>
        </div>
        -->

        <div v-if="$store.state.total_amount_order_paid>0"  class="grid grid-cols-12 mt-1 bg-gray-100 border-l-4 border-green-700">
            <div class="col-span-3 p-2 "><b class="text-green-600">Pagado</b></div>
            <div class="col-span-4 p-2"></div>
            <div class="col-span-4 p-2 text-right"><b class="text-green-600">$ {{ $store.state.total_amount_order_paid + $store.state.tip}}</b></div>
            <div class="col-span-1 pt-2 pb-2 pr-2 text-center ">
                <svg @click = "openModalPaid" class="w-6 h-6 text-green-500 hover:text-green-700" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <div v-if="$store.state.discount_active>0"  class="grid grid-cols-12 mt-1 bg-gray-100 border-l-4 border-red-700 hover:bg-red-300">
        ...
        <div v-if="$store.state.discount_active_percentage>0"  class="grid grid-cols-12 mt-1 bg-gray-100 border-l-4 border-red-700 hover:bg-red-300">
            <div class="col-span-3 p-2 text-xs lg:text-lg"><b class="text-red-600">Desc %</b></div>
            <div class="col-span-5 p-2 lg:col-span-5"></div>
            <div class="col-span-3 p-2 text-right text-xs lg:text-lg"><b class="text-red-600">$ -{{ $store.state.discount_active_percentage }}</b></div>
            <div class="col-span-1 pt-2 pb-2 pr-2 discount">
